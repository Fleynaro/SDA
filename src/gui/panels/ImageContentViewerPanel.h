				...
				void renderPanel() override {
					...
					if (const auto emulator = m_codeSectionViewer->m_projectPanel->getEmulator()) {
						...
						if (ImGui::MenuItem("Start Emulator here")) {
							const auto stepWidth = m_codeSectionRow.m_isPCode ? PCodeEmulator::PCodeStepWidth::STEP_PCODE_INSTR : PCodeEmulator::PCodeStepWidth::STEP_ORIGINAL_INSTR;
				...
				if (ImGui::BeginMenu("Debug"))
				{
					if (codeSectionViewer->m_curFuncPCodeGraph) {
						if (ImGui::MenuItem("Start Emulator")) {
							m_projectPanel->createEmulator(m_imageDec, codeSectionViewer->m_curFuncPCodeGraph->getStartBlock()->getMinOffset(), PCodeEmulator::PCodeStepWidth::STEP_ORIGINAL_INSTR);
						}
					}
		...
		void processDecompiledCodeViewerEvents() {
			...
			if (const auto decCodeViewerPanel = dynamic_cast<DecompiledCodeViewerPanel*>(m_decompiledCodeViewerWindow->getPanel())) {
				...
				// start debug
					...
					m_projectPanel->createEmulator(m_imageDec, m_curDecGraph->getStartBlock()->m_pcodeBlock->getMinOffset(), PCodeEmulator::PCodeStepWidth::STEP_CODE_LINE);
				}
			}
		}
	};
